...
var UI = {
    init: function init(rootElement, steadHandler) {
        ...
        this.element.$inventory.on('touchstart', 'a', function handler(e) {
            var orig = e.originalEvent;
            orig.preventDefault();
            var x = orig.changedTouches[0].pageX;
            var y = orig.changedTouches[0].pageY;
            $('#DRAG').css({top: y, left: x}).attr('data-ref', $(e.target).attr('data-ref'));
        });
        this.element.$inventory.on('touchmove', 'a', function handler(e) {
            var orig = e.originalEvent;
            orig.preventDefault();
            var x = orig.changedTouches[0].pageX;
            var y = orig.changedTouches[0].pageY;
            $('#DRAG').css({top: y, left: x}).show().text(e.target.text);
            $('#stead a').removeClass('hover');
            var elem = document.elementFromPoint(x, y);
            if (elem && $(elem).is('a')) {
                $(elem).addClass('hover');
            }
        });
        this.element.$inventory.on('touchend', 'a', function handler(e) {
            e.preventDefault();
            $('#DRAG').hide();
            var orig = e.originalEvent;
            var x = orig.changedTouches[0].pageX;
            var y = orig.changedTouches[0].pageY;
            var elem = document.elementFromPoint(x, y);
            steadHandler.drag($('#DRAG').attr('data-ref'), $(elem).attr('data-ref'));
            $('#stead a').removeClass('hover');
        });
